...
def create_simple_config(model_name: str = None) -> AIConfig:
    ...
    from app.aicore.config.model_config import ModelProvider
    from app.aicore.config.runner_config import StreamingMode
...
def display_configuration(client: ConfigurableAssistantClient, conversation_history=None):
    ...
    tools = client.assistant.agent.tools
    #print(f"Configurable Agent Tools: {tools}")
    tool_names = []
    for tool in tools:
        tool_names.append(tool.name)
        
    print(f"  Configurable Agent Tools: {COLORS['green']}{', '.join(tool_names)}{COLORS['reset']}")
    print()

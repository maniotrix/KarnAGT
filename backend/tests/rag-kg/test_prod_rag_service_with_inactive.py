...
class ProductionRAGTestRunner:
    ...
    async def run_comprehensive_test(self):
        ...
        try:
            ...
            # === PHASE 5: Document Status Filtering Test ===
            print("\n📋 PHASE 5: Document Status Filtering Test")
            print("-" * 50)
            
            # Mark first document (train ticket) as inactive
            print(f"🔒 Marking first document as inactive...")
            print(f"   📄 Documents to deactivate: {ref_doc_ids_1}")
            
            async for db in get_db():
                inactive_count = await self.prod_rag_service.mark_documents_inactive(
                    collection_id=collection_id,
                    doc_ids=ref_doc_ids_1,
                    db=db
                )
                if inactive_count != len(ref_doc_ids_1):
                    raise Exception(f"Expected {len(ref_doc_ids_1)} documents to be marked as inactive, but got {inactive_count}")
                print(f"   ✅ {inactive_count} documents marked as inactive")
                break
            
            # Test that whole collection queries now only return active documents
            print("🔍 Testing whole collection queries (should only return active documents)...")
            ...
            async for db in get_db():
                for i, query in enumerate(status_test_queries, 1):
                    print(f"\n📋 Status Test Query {i}: {query}")
                    
                    start_time = time.time()
                    
                    # Query whole collection without document filtering (should only return active docs)
                    query_result = await self.prod_rag_service.query_collection(
                        collection_id=collection_id,
                        query=query,
                        user_id=self.test_user_id,
                        db=db,
                        include_inactive=False  # Only active documents
                    )
                    ...
                    # Analyze which documents were returned
                    active_docs_found = []
                    inactive_docs_found = []
                    
                    for source in query_result.sources:
                        ref_doc_id = source.get('ref_doc_id')
                        if ref_doc_id in ref_doc_ids_1:
                            inactive_docs_found.append(ref_doc_id)
                        elif ref_doc_id in ref_doc_ids_2:
                            active_docs_found.append(ref_doc_id)
                    
                    print(f"🟢 Active documents found: {len(active_docs_found)} (from Trykaa doc)")
                    print(f"🔴 Inactive documents found: {len(inactive_docs_found)} (from train ticket - should be 0)")
                    
                    # Show sources with status indication
                    for j, source in enumerate(query_result.sources[:3], 1):
                        ref_doc_id = source.get('ref_doc_id')
                        if ref_doc_id in ref_doc_ids_1:
                            status_icon = "🔴 INACTIVE (ERROR!)"
                        elif ref_doc_id in ref_doc_ids_2:
                            status_icon = "🟢 ACTIVE"
                        else:
                            status_icon = "⚪ UNKNOWN"
                        
                        print(f"   {j}. {source.get('file_name', 'Unknown')} {status_icon}")
                        print(f"      📝 {source.get('text_preview', 'No preview')}")
                        print(f"      🆔 ref_doc_id: {ref_doc_id}")
                    
                    # Validate that no inactive documents were returned
                    if inactive_docs_found:
                        print(f"❌ ERROR: Found {len(inactive_docs_found)} inactive documents in results!")
                        print(f"   Inactive docs returned: {inactive_docs_found}")
                        raise Exception("Status filtering failed - inactive documents were returned!")
                    else:
                        print(f"✅ SUCCESS: No inactive documents returned")
                    
                    # Validate that we still get results from active documents
                    if not active_docs_found:
                        print(f"⚠️  WARNING: No active documents found - collection may be empty")
                    else:
                        print(f"✅ SUCCESS: Found {len(active_docs_found)} results from active documents")
                
                break
            
            # Test include_inactive=True to show that inactive docs are still there
            print(f"\n🔍 Testing include_inactive=True (should return all documents)...")
            
            async for db in get_db():
                start_time = time.time()
                query_result = await self.prod_rag_service.query_collection(
                    collection_id=collection_id,
                    query=status_test_queries[0],
                    user_id=self.test_user_id,
                    db=db,
                ...
                print(f"📊 With include_inactive=True:")
                print(f"   🟢 Active documents: {len(active_docs_found)}")
                print(f"   🔴 Inactive documents: {len(inactive_docs_found)}")
                print(f"   📚 Total sources: {len(query_result.sources)}")
                
                if inactive_docs_found:
                    print(f"✅ SUCCESS: Inactive documents are still accessible when explicitly requested")
                else:
                    print(f"⚠️  WARNING: No inactive documents found even with include_inactive=True")
                
                break
            
            print(f"\n🎯 DOCUMENT STATUS FILTERING TEST COMPLETED!")
            print(f"   ✅ Inactive documents filtered out by default")
            print(f"   ✅ Inactive documents accessible when explicitly requested")
            print(f"   ✅ Active documents still queryable")
            print(f"   ✅ Status-based filtering working correctly")
